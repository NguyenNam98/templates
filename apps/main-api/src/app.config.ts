...
const getApplicationConfig = async (
  ...
  try {
    ...
    if (!listProfiles) {
      throw new Error(
        '++++++++++++++++++++++++++ No profiles found for the application: ',
      )
    }

    const profile = listProfiles.find(
      (profile) => profile.Name === `${appName}.${env}`,
    )
...
const getEnvVariables = async (): Promise<IConfiguration> => {
  try {
    ...
    const { serviceConfVersion } = configVersions
    ...
    if (env === ENV_LOCAL && fs.existsSync(envLocalPath)) {
      const existConfigLocal = (jsYaml.loadAll(
        fs.readFileSync(envLocalPath, 'utf-8'),
      )[0] || {}) as IConfiguration
      if (existConfigLocal[SERVICE_CONF_VERSION] == versionService) {
    ...
    const [serviceConfig] = await Promise.all([
